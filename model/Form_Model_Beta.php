...
class Form_Model_Beta extends OptionModelSub\Form_Model_Alpha {
  ...
  PRIMARY KEY (`ID`)
        ...
        // add column
        function find_missing_tables($database, $column, $details) {
        ...
        // alter column
        function alter_columns($database, $column, $details, $charset) {
        ...
        alter_columns($columns, 'form_body', 'text', 'char(250)');
    ...
    protected function create_post_items($var, $bol = TRUE) {
                ...
                foreach ($total as $result => $value) {
                    ...
                    foreach ($value as $key => $row_value) {
                        ...
                        if ($item->form_categories_parent != "") {
                            $cat_ID = $this->find_cat_id($item->form_categories_parent);
                            $form_categories_parent = (int)$cat_ID->term_id;
                        } else {
                        ...
                        if ($key === ($total_val - 1)) {
                            ...
                            $new_post['post_content'] = stripslashes_deep($this->add_tracking_link($this->
                                check_utf($post_content)));
                            ...
                            if ($item->post_status !== NULL || $item->post_status !== "") {
                                                ...
                                                if (!preg_match("/^[0-9]/", $result)) {
                                                    ...
                                                    if (mb_strlen($result) > 200) {
                                                        $id = wp_create_category($this->check_utf(mb_substr
                                                            ($result, 0, 200)), $form_categories_parent);
                                                    } else {
        ...
        if (isset($total_cats)) {
            $this->write_file($total_cats." posts from ".$item->name." were updated on ".date('h:i:s A, l jS \of F Y'));
        }

        if (isset($total_new)) {
            $this->write_file($total_new." posts from ".$item->name." were created on ".date('h:i:s A, l jS \of F Y'));
        }
    ...
    private function dont_follow_links($html) {
    ...
    protected function synchronize_feeds($var) {
        ...
        if (isset($total_delete)) {
            $this->write_file($total_delete." posts from ".$_GET['unique_form'].
    ...
    protected function find_file_name() {
        ...
        return $wpdb->get_results("SELECT fileName FROM ".AH_FEED_DETAILS_TABLE." WHERE name <> '".
            $_GET['unique_name']."'");
...
WHERE a.post_type = 'revision' AND post_title = %s", $result->post_title));
    ...
    protected function update_ind_form($form, $form_name) {
        ...
        if ($this->update_feed_details($form_title, $form_title_contains, $form_body, $form_body_contains,
            ...
            $form_max_rows, $form_post_status, $form_posttype, $form_nofollow, $form_cat_parents)) {
    ...
    private function find_cat_id($var) {

        global $wpdb;

        return $wpdb->get_row("SELECT term_id FROM ".$wpdb->prefix."terms WHERE name  = '".$var."'");
    ...
    protected function update_feed_details($form_title, $form_title_contains, $form_body, $form_body_contains,
        ...
        return $wpdb->query($wpdb->prepare("UPDATE ".AH_FEED_DETAILS_TABLE.
            " SET form_title = %s, form_title_contains = %s, form_body = %s, form_body_contains = %s, form_categories = %s, form_tags = %s, form_allow_comments = %d, form_allow_trackback = %d, min_rows = %d, max_rows = %d, post_status = %s, post_type = %s, form_body_nofollow = %d, form_categories_parent = %s WHERE name = %s",
            ...
            $form_posttype, $form_nofollow, $form_cat_parents, $form_name));
    }
    ...
    protected function update_record($form) {
        ...
        if ($this->get_file_extension($fileName) === "xml") {
            ...
            keep!! REMOVES ALL EMPTY TAGS AND SELF-ENCLOSED TAGS
            ...
            $fh = fopen(AH_FEEDS_DIR.$fileName, 'w+');
            fwrite($fh, $finalXML);
            fclose($fh);
